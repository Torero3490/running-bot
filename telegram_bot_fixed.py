...
async def handle_all_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if current_hour >= 22 or current_hour < 6:
            ...
            # Отправляем предупреждение после 3 сообщений
            if user_night_messages[user_id] == 3 and user_id not in user_night_warning_sent:
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "Привет! Я бот бегового сообщества.\n\n"
        "Доступные команды:\n"
        "/facts — факт о беге\n"
        "/slots — мероприятия и слоты\n"
        "/birthday DD.MM — задать день рождения\n"
        "/challenge — статус челленджа\n"
        "/votechallenges — голосование за челлендж (админы)\n"
        "\nЕсли что-то не работает — попробуйте позже."
    )
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=text,
    ...
    try:
        ...
        if not events:
            await context.bot.edit_message_text(
                ...
                text="Сейчас нет доступных слотов на мероприятия.",
            )
            return

        max_items = 20
        lines = ["🏃 <b>Доступные слоты на мероприятия:</b>\n"]
        for idx, event in enumerate(events[:max_items], start=1):
            ...
            lines.append(f"{idx}. <b>{title}</b> — {date} — {city}")
            if url:
                lines.append(f'<a href="{url}">{url}</a>')
            lines.append("")

        if len(events) > max_items:
            lines.append(f"И еще {len(events) - max_items} мероприятий. Напиши /slots позже для обновления.")
...
def register_handlers(app):
    ...
    app.add_handler(CommandHandler("start", start_cmd, block=False))
    app.add_handler(CommandHandler("help", help_cmd, block=False))
    app.add_handler(CommandHandler("facts", facts_cmd, block=False))
    ...
    app.add_handler(CommandHandler("slots", slots_cmd, block=False))
    ...
    app.add_handler(MessageHandler(filters.COMMAND, unknown_command, block=False), group=2)
    app.add_handler(MessageHandler(filters.ALL, handle_all_messages), group=1)


def start_background_threads():
    """Запускает фоновые потоки (Flask, events scheduler)."""
...
async def post_init(app):
    ...
    app.create_task(facts_scheduler_task())
    app.create_task(birthday_scheduler_task())
    app.create_task(morning_scheduler_task())
    app.create_task(coffee_scheduler_task())
    app.create_task(lunch_scheduler_task())
    app.create_task(motivation_scheduler_task())
    app.create_task(daily_summary_scheduler_task())
    app.create_task(garmin_scheduler_task())

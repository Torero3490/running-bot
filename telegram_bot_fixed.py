...
def get_music_of_day() -> str:
    """Возвращает трек дня по дате (стабильно на день)."""
    today_key = datetime.now(MOSCOW_TZ).strftime("%Y-%m-%d")
    idx = abs(hash(today_key)) % len(MUSIC_OF_DAY)
...
async def send_morning_greeting():
    ...
    try:
        ...
        music_of_day = get_music_of_day()
        greeting_text = (
            f"🌅 **Доброе утро, бегуны!** 🏃‍♂️\n\n"
            f"{weather}\n\n"
            f"{theme}\n\n"
            f"🎵 *Музыка дня:* {music_of_day}\n\n"
        )
    ...
    while bot_running:
        ...
        if current_hour == 22 and current_minute == 0:
            if good_night_sent_date != today_date:
                logger.info("Время 22:00 - отправляем спокойной ночи")
                try:
...
async def handle_all_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Если это reply на сообщение бота — отвечаем
...
async def unknown_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text="Команда не распознана. Напишите /help для списка команд.",
    )
...
async def music_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=f"🎵 Музыка дня: {get_music_of_day()}",
    )
...
async def garmin_list_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id, data in garmin_users.items():
        ...
        lines.append(f"• {escape_markdown(data.get('name', 'Unknown'))} — {masked_email}")
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text="\n".join(lines),
        parse_mode="Markdown",
...
async def post_init(app):
    ...
    app.create_task(good_night_scheduler_task())

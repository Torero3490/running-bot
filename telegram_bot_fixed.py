...
async def slots_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        max_items = 20
        lines = ["🏃 *Доступные слоты на мероприятия:*\n"]
        for idx, event in enumerate(events[:max_items], start=1):
            title = escape_markdown(event.get("title", "Без названия"))
            date = escape_markdown(event.get("date", "Дата не указана"))
            city = escape_markdown(event.get("city", "Город не указан"))

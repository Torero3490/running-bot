...
def get_event_region(event: Dict) -> tuple:
    """
    Определяет регион события (Москва, СПб, Ижевск).
    Возвращает кортеж (is_moscow, is_spb, is_izhevsk).
    Использует полный text_blob для надёжного определения.
    """
    city = (event.get('city', '') or '').lower()
    title = (event.get('title', '') or '').lower()
    location = (event.get('location', '') or '').lower()
    ...
    spb_region_keywords = [
        'санкт-петербург', 'saint petersburg', 'st. petersburg',
        'петербург', 'питер', 'спб', 'spb',
        'ленинградск', 'ленинградской', 'ленинградская', 'ленинград', 'ло',
        'гатчина', 'выборг', 'всеволожск', 'тосно', 'пушкин',
        'pulkovo', 'пулково', 'царское село',
        'scarlet', 'алые паруса', 'алые', 'паруса'
    ]
    ...
    izhevsk_region_keywords = [
        'ижевск', 'udmurt', 'удмурт', 'удмуртск', 'ижевская', 'ижевской', 'удмуртия'
    ]
    ...
    return (is_moscow, is_spb, is_izhevsk)
...
async def get_all_events() -> List[Dict]:
    ...
    for event in all_events:
        ...
        if filter_event_by_year_and_city(event):
            filtered_events.append(event)
            # Подсчитываем по регионам (используем единую функцию)
            is_moscow, is_spb, is_izhevsk = get_event_region(event)
            if is_moscow:
                moscow_count += 1
            if is_spb:
                spb_count += 1
    ...
    for event in filtered_events:
        ...
        # Определяем регион события для статистики (используем единую функцию)
        is_moscow_event, is_spb_event, is_izhevsk_event = get_event_region(event)
            ...
            async with httpx.AsyncClient(timeout=15.0, follow_redirects=True) as client:
                ...
                if is_open is False:
                    ...
                    if is_moscow_event:
                        ...
                        logger.info(f"[EVENTS] СПб/ЛО - регистрация закрыта: {event.get('title', 'Без названия')} - {url}")
                    ...
                    if is_moscow_event:
                        ...
                        spb_open += 1
                        logger.info(f"[EVENTS] ✅ СПб/ЛО - регистрация открыта: {event.get('title', 'Без названия')} - {url}")
                    elif is_izhevsk_event:
                        izhevsk_open += 1
                        logger.info(f"[EVENTS] ✅ Ижевск/Удмуртия - регистрация открыта: {event.get('title', 'Без названия')} - {url}")
        except Exception as e:

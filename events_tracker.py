...
def filter_event_by_year_and_city(event: Dict) -> bool:
    ...
    # Если год не найден (0) - не фильтруем по году
    if year == 0:
        logger.info(f"[EVENTS] Год не определён, не фильтруем: {event.get('title', 'Без названия')}")
        return True
...
def filter_event_by_city_only(event: Dict) -> bool:
    ...
    # Если город не определён (пустой или "Россия") — пропускаем как подходящий
...
async def get_all_events() -> List[Dict]:
    ...
    # Если по строгим фильтрам ничего не найдено, делаем мягкую фильтрацию только по региону
    if not filtered_events and all_events:
        logger.warning("[EVENTS] Нет событий по фильтру года, пробуем фильтр только по региону")
        relaxed_events: List[Dict] = []
        for event in all_events:
            if filter_event_by_city_only(event):
                if 'link' not in event and 'url' in event:
                    event['link'] = event.get('url', '')
